...
export default function HeaderAndBody() {
  ...
  // Search for Products from Filtered Products (from Input)
  let searchFilteredItems = (inputVal) => {
    let filteredList = [];
    selectedCategoryArr.forEach((y) => {
      filteredList = [
        ...filteredList,
        ...productList.filter((x) =>
          x.category == y && x.title.toLowerCase().includes(inputVal.toLowerCase())
        ),
      ];
    });
...
// It runs after the initialization of Component
  useEffect(() => {
    getCategories();
  }, []);
    ...
    <>
      ...
      <AppBar position="sticky" sx={{ backgroundColor: "#fff", color: "#333333" }}>
        ...
        <Toolbar sx={{ display: "flex", justifyContent: "space-between", alignItems: "center", flexWrap: "wrap" }}>
          ...
          <Box sx={{ width: "50%" }}>
            ...
            >
              <InputBase
                ...
                onChange={(e) => searchFilteredItems(e.target.value)}
              />
          ...
          <Box sx={{ width: "20%", display: "flex", justifyContent: "space-between", alignItems: "center", flexWrap: "wrap" }}>
          ...
          <Slider {...settings}>
            ...
            {selectedCategoryArr && selectedCategoryArr.length > 0
              ? selectedCategoryArr.map((x, i) => (
                <Chip
                  sx={{ width: "fit-content !important", bgcolor: '#FF6A00', color: 'white' }}
                  color={"primary"}
                  key={i}
